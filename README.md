# ACCENTURE-BOOTCAMP
BOOTCAMP
SUMMARY OF DOCKER AND KUBERNETS

Today I worked with Kubernetes using kubectl and Minikube on an EC2 instance. First, I checked the Kubernetes version and verified that the cluster was running properly. I created and managed pods using the nginx image, accessed the pod terminal, and learned how to view, describe, and delete pods.
Next, I created a Deployment to manage multiple nginx pods automatically. I scaled the deployment to run multiple replicas and understood how Kubernetes maintains the desired number of pods. I also created a ConfigMap and Secret to store configuration values and sensitive data, and connected them to the deployment as environment variables.
After that, I created a Service of type NodePort to expose the nginx application so it could be accessed through a browser using the Minikube IP and NodePort. Finally, I verified the setup by accessing the pod, printing environment variables, and confirming that the deployment, service, ConfigMap, and Secret were working correctly
...
kubectl exec -it <pod-name> -- /bin/sh – Opens shell inside running pod.
kubectl exec -it <pod-name> -- /bin/sh -c 'echo $WELCOME_MSG' – Displays ConfigMap value inside pod.
kubectl exec -it <pod-name> -- /bin/sh -c 'echo $ADMIN_PASSWORD' – Displays Secret value inside pod.
...
CONCLUSION
This task helped me understand how Kubernetes manages applications using Pods, Deployments, Services, ConfigMaps, and Secrets
